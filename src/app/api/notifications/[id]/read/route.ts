...
export const PATCH = withErrorHandling(async (_req: Request, { params }: { params: { id: string } }) => {
  ...
  if (!notification) throw new ApiException(404, "Notification not found.");
  if (notification.userId !== user.id) throw new ApiException(403, "You can only update your own notifications.");
